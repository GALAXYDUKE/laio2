...
//            Toast.makeText(context, "当前网络类型为WIFI", Toast.LENGTH_LONG).show();
    ...
    public static Client getInstance(Context context, ISocketResponse respListener) {
        if (client == null)
            client = new Client(context, respListener);
        return client;
    }

    private Client(Context context, ISocketResponse respListener) {
    ...
    public synchronized void reconn(String fromwhere) {
        if (isNeedConn()) {
            IP = ServerConfigall.serverip;
            PORT = ServerConfigall.serverport;
